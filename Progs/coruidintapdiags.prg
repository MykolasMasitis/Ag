PROCEDURE CorUIDInTapDiags
 ...
 WAIT "..." WINDOW NOWAIT 
 SelString = "SELECT TOP 100 uid FROM t_tapdiagnosis_31571 ORDER BY uid DESC"

 lrslt = SQLEXEC(lnHndl, SelString)
 IF lrslt<=0
  = MESSAGEBOX('Cannot process select!', 16, '')
 ELSE 
  CREATE TABLE &tc_comdir\tapdiagsuid ("row" i, uid i, next_uid i) 
  USE 
  =OpenFile(tc_comdir+'\tapdiagsuid', 'tapdiagsuid', 'excl')
  SELECT sqlresult
  BROWSE 
  SCAN 
   m.uid    = uid 
   ...
   INSERT INTO tapdiagsuid FROM MEMVAR 
