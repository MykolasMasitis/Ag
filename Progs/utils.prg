...
FUNCTION FSumm(Usl, STip, Kol, IsVed)
 IF SEEK(Usl, 'Tarif')
  m.n_kd    = Tarif.n_kd
   ...
   do case
    case int(Usl/1000) = 83
     do case
      ...
      case kol = m.n_kd
       summa = m.price
      case kol > m.n_kd and kol <= 30
       summa = round(m.kd_price * kol,2)
      case kol > m.n_kd and kol > 30
       summa = round(m.kd_price * 30,2)
     endc 
    case int(Usl/1000) = 183
     summa = iif(Kol<=30, Kol*m.price, 30*m.price)
    othe
     IF INLIST(STip,'Д','П')
      summa = m.price
     ELSE
...
FUNCTION PDFTYPETEXT(m.LINEOFTEXT)
...
FUNCTION IsCorrectDate(DateToCheck)
 m.DateToCheck = ALLTRIM(DateToCheck)
 IF LEN(m.DateToCheck)<10
  RETURN .f. 
 ENDIF 
 IF !BETWEEN(INT(VAL(SUBSTR(m.DateToCheck,1,4))), 1994, YEAR(DATE())) && Year
  RETURN .f. 
 ENDIF 
 IF !BETWEEN(INT(VAL(SUBSTR(m.DateToCheck,6,2))), 01, 12) && Month
  RETURN .f. 
 ENDIF 
 IF !BETWEEN(INT(VAL(SUBSTR(m.DateToCheck,9,2))), 01, 31) && Day
  RETURN .f. 
 ENDIF 
RETURN .T.
